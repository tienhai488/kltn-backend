<x-base-layout :scrollspy="false">
    ...
    <x-custom.breadcrumb
        :breadcrumb-items="[
            ...
            __('Danh sách yêu cầu tài khoản cá nhân') => route('admin.organization_account_request.index'),
            __('Chi tiết yêu cầu tài khoản cá nhân') => ''
        ]"/>
    <x-custom.stat-box :id="'organization-management'" :custom-col="'col-lg-12'">
        <x-slot:boxTitle>
            <div class="d-flex justify-content-between align-items-center">
                ...
                <a
                    href="{{ route('admin.organization_account_request.create', ['id' => $individualAccountRequest->id]) }}"
        ...
        <div class="col-xl-12 row" style="padding: 16px 15px 0; border-top: 1px solid #e3e6f0;">
            <div>
                <div class="form-group row">
                    <label for="" class="col-sm-2 col-form-label col-form-label-sm">{{ __('Tên cá nhân') }}:</label>
                    <div class="col-sm-10 d-flex align-items-center">
                        <p> {{ $individualAccountRequest->name }} </p>
                    </div>
                </div>
                <div class="form-group row">
                    <label for="" class="col-sm-2 col-form-label col-form-label-sm">{{ __('Ngày thành lập') }}:</label>
                    <div class="col-sm-10 d-flex align-items-center">
                        <p> {{ customFormatDate($individualAccountRequest->birth, 'd/m/Y') }} </p>
                    </div>
                </div>
                <div class="form-group row">
                    <label for="" class="col-sm-2 col-form-label col-form-label-sm">{{ __('Website') }}:</label>
                    <div class="col-sm-10 d-flex align-items-center">
                        <a href="{{ $individualAccountRequest->website }}" target="_blank" class="text-primary">
                            {{ $individualAccountRequest->website }}
                ...
                <div class="text-anywhere">
                    <p>{{ $individualAccountRequest->information }}</p>
                </div>
            </div>
        </div>
    </x-custom.stat-box>
    ...
    @can(Acl::PERMISSION_ACCOUNT_REQUEST_EDIT)
    <input type="hidden" id="individual-account-request-status" value="{{ $individualAccountRequest->status }}">

    <livewire:admin.individual-account-request.partials.edit-individual-account-request-status-modal
        :individual-account-request="$individualAccountRequest"
    />
    @endcan
