<x-base-layout :scrollspy="false">
    ...
    <x-custom.stat-box :id="'organization-management'" :custom-col="'col-lg-12'">
        ...
        <div class="col-xl-12 row" style="padding: 16px 15px 0; border-top: 1px solid #e3e6f0;">
            <div>
                ...
                <div class="form-group row">
                    <label for="" class="col-sm-2 col-form-label col-form-label-sm">{{ __('Ngày sinh') }}:</label>
                    <div class="col-sm-10 d-flex align-items-center">
                        <p> {{ customFormatDate($individual->birth, 'd/m/Y') }} </p>
                    </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group row">
                    <label for="" class="col-sm-2 col-form-label col-form-label-sm">{{ __('Câu lạc bộ') }}:</label>
                    <div class="col-sm-10 d-flex align-items-center">
                        <p> {{ $individual->club_name }} </p>
                    </div>
                </div>
                <div class="form-group row">
                    <label for="" class="col-sm-2 col-form-label col-form-label-sm">{{ __('Lĩnh vực hoạt động') }}:</label>
                    <div class="col-sm-10 d-flex align-items-center">
                        <p> {{ $individual->field }} </p>
                    </div>
                </div>
                <div class="form-group row">
                    <label for="" class="col-sm-2 col-form-label col-form-label-sm">{{ __('Website') }}:</label>
                    <div class="col-sm-10 d-flex align-items-center">
                        <a href="{{ $individual->website }}" target="_blank" class="text-primary">
                            {{ $individual->website }}
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10 d-flex align-items-center">
                        <span class="badge badge-{{ $individual->status->getBadge() }}">{{ $individual->status->getLabel() }}</span>
                    </div>
                </div>
                @if (count($individual->related_images))
                                                ...
                                                <div class="splide-mainThubnail splide--fade splide--ltr splide--draggable is-active is-initialized" id="splide04">
                                                    <div class="splide__track" id="splide04-track" style="padding-left: 0px; padding-right: 0px;">
                                                        <ul class="splide__list" id="splide04-list">
                                                            @foreach ($individual->related_images as $index => $image)
                                                            <li
    ...
    @can(Acl::PERMISSION_ACCOUNT_REQUEST_EDIT)
    <input type="hidden" id="individual-account-request-status" value="{{ $individual->status }}">

    <livewire:admin.individual-account-request.partials.edit-individual-account-request-status-modal
        :individual-account-request="$individual"
    />
    @endcan
