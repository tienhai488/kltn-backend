<x-base-layout :scrollspy="false">
    ...
    <x-slot:pageTitle>
        {{ __('Quản lý yêu cầu tài khoản tổ chức') }}
    </x-slot:pageTitle>
    ...
    <x-custom.breadcrumb :breadcrumb-items="['yêu cầu tài khoản tổ chức' => '', 'Quản lý yêu cầu tài khoản tổ chức' => '']"/>

    <x-custom.stat-box :id="'organization-management-filter'" :custom-col="'col-lg-12'">
        <x-slot:boxTitle>
            {{ __('Bộ lọc') }}
        </x-slot:boxTitle>

        @include('admin.organization_account_request.filters.index')
    </x-custom.stat-box>
        ...
        >
            <x-slot:tableHeader>
                <tr>
                    ...
                    <th>{{ __('Tên tổ chức') }}</th>
                    <th>{{ __('Ngày thành lập') }}</th>
                    <th>{{ __('Lĩnh vực hoạt động') }}</th>
                    <th>{{ __('Thông tin người đại diện') }}</th>
                    <th>{{ __('Trạng thái') }}</th>
                        ...
                        "render": function (data, type, full) {
                            return `
                                ...
                                <div class="d-flex">
                                    ...
                                    <p class="text-primary">${full.representative_email}</p>
                                </div>
                                <div class="d-flex">
                                    <p class="text-start me-1">{{ __('Số điện thoại') }}:</p>
                                    <p class="text-primary">${full.representative_phone_number}</p>
                                </div>
                            `;
                        }
                    },
