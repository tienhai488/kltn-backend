<div class="row">
    <div id="users-box" class="col-lg-12 layout-spacing">
        <div wire:loading.remove class="statbox widget box box-shadow">
            <div id="chart-donation"></div>
            <div id="chart-donation-sum-amount"></div>
        </div>
    </div>
    <div wire:loading>
        <div id="users-box" class="col-lg-12 layout-spacing">
    ...
    @script
    <script>
        Livewire.on('dispatch-for-chart-donation', () => {
                ...
                let updateLineChart = () => {
                    ...
                    $wire.getData(range, projectId).then(data => {
                        if (data) {
                                    ...
                                    labels: {
                                        formatter: function (val) {
                                            if (typeof val != "undefined") {
                                                return val.toLocaleString('en') + 'đ';
                                            }
                                            return val;
                                        }
                                    }
                            ...
                            const sumAmount = chartData.reduce((sum, item) => sum + item, 0);
                            const dateRange = `${range[0]} - ${range[range.length - 1]}`;
                            document.querySelector('#chart-donation-sum-amount').innerHTML = `
                                <h6>{{ __('Tổng số tiền quyên góp') }}: ${sumAmount.toLocaleString('en') + 'đ'}</h6>
