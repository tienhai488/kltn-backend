<x-member.base-layout :scrollspy="false">
    ...
    <x-custom.breadcrumb :breadcrumb-items="['tình nguyện viên' => '', 'Quản lý tình nguyện viên' => '']"/>

    <x-custom.stat-box :id="'volunteer-management-filter'" :custom-col="'col-lg-12'">
        <x-slot:boxTitle>
            {{ __('Bộ lọc') }}
        </x-slot:boxTitle>

        @include('member.volunteer.filters.index')
    </x-custom.stat-box>
            ...
            <x-slot:customScript>
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "orderable": false,
                        "render": function (data, type, full) {
                            let urlShow = `{{ route('member.volunteer.show', ':id') }}`.replace(':id', data);

                            return `
                                <ul class="table-controls d-flex justify-content-center">
                                    <x-table.actions.show-action
                                        :permission="Acl::PERMISSION_VOLUNTEER_LIST"
                                        :url="'${urlShow}'"
                                    />
                                </ul>`;
                            }
    ...
    <x-slot:footerFiles>
        ...
        <script>
            function handleExport(isStudent = false) {
                    ...
                    let exportUrl = "{{ route('member.volunteer.export') }}";
